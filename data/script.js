...
function onMessage(event) {
    ...
    try {
        ...
        // Check if data contains temperature
        if (data.temperature !== undefined) {
            gaugeTemp.refresh(data.temperature);
        }
        // Check if data contains humidity
        if (data.humidity !== undefined) {
            gaugeHumi.refresh(data.humidity);
        }

        //check if data contains tinyml_result
        if (data.tinyml_result !== undefined) {
            // Assuming tinyml_result is between 0 and 1, convert to percentage
            let anomalyPercentage = data.tinyml_result * 100;
            gaugeTinyML.refresh(anomalyPercentage);
        }
...
window.onload = function () {
    ...
    gaugeTinyML = new JustGage({
        id: "gauge_tinyml",
        value: 0,
        min: 0,
        max: 100, // Adjust this based on your model's output range (e.g., if 0-1, multiply by 100)
        donut: true,
        pointer: true,
        gaugeWidthScale: 0.25,
        gaugeColor: "transparent",
        levelColorsGradient: true,
        levelColors: ["#00FF00", "#FFFF00", "#FF0000"], // Green to Red (Anomaly)
        label: "Anomaly %"
    });
    // --- NEW CODE END ---
